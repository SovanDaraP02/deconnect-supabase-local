...
// Purpose: Background cleanup tasks (triggered by cron or manually)
...
serve(async (req) => {
  ...
  let payload: { task?: string; days_old?: number };
  try {
    payload = await req.json();
  } catch {
    payload = { task: "all", days_old: 7 };
  }

  const { task = "all", days_old = 7 } = payload;
  ...
  try {
    ...
    if (task === "devices" || task === "all") {
      ...
      const { data, error } = await supabase
        ...
        .delete()
        .eq("is_active", false)
        .lt("last_active_at", deviceCutoff.toISOString())
        .select("id");
      results.push({
    ...
    // Task 5: Clean old analytics events (30+ days)
    if (task === "all") {
    ...
    const totalDeleted = results.reduce(
      (sum, r) => sum + (r.deleted_count as number),
      0,
    );
    await supabase.from("system_logs").insert({
      level: "info",
      message: "Cleanup worker completed",
      metadata: { task, days_old, results, total_deleted: totalDeleted },
    });
    ...
    return json(500, { error: "Cleanup failed", details: msg });
  }
});
